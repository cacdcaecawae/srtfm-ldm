...
class DDPM():
    ...
    def sample_back_onestep(self, x_t, t, eps):
        alpha_bar = self.alpha_bars[t].reshape(-1, 1, 1, 1)
        res = (x_t - torch.sqrt(1 - alpha_bar) * eps) / torch.sqrt(alpha_bar)
        return res
    ...
    def sample_backward_sr(self, img_shape, net, lr_image, device, simple_var=True):
        ...
        net = net.to(device)
        for t in range(self.n_steps - 1, -1, -1):
            x = self.sample_backward_stepsr(x, t, net, lr_image, simple_var)
        return x

    def sample_backward_stepsr(self, x_t, t, net, lr_image, simple_var=True):

        n = x_t.shape[0]
        t_tensor = torch.tensor([t] * n,
                                dtype=torch.long).to(x_t.device).unsqueeze(1)
        eps = net(x_t, t_tensor, lr_image)
