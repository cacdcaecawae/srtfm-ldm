...
def evaluate(cfg: Dict[str, Any], device: torch.device, two_stage: bool = True) -> None:
    ...
    with torch.inference_mode():
        for lr_images, hr_images, names in tqdm(dataloader, desc="Evaluating"):
            ...
            expanded_shape = (batch_size * n_samples,) + img_shape[1:]
            condition_expanded = condition.repeat(n_samples, 1, 1, 1)
            
            if sampler_type == "ddim":
                sr_images_all = ddpm.sample_backward_sr(expanded_shape,
                                                        ddpm_net,
                                                        condition_expanded,
                                                        device=device,
            ...
            sr_images_reshaped = sr_images_all.view(n_samples, batch_size, *img_shape[1:])
    ...
    print(f"Dice - Average: {avg_dice:.4f}, Max: {max_dice:.4f}, Min: {min_dice:.4f}")
    print(f"HD95 - Average: {avg_hd95:.4f}, Max: {max_hd95:.4f}, Min: {min_hd95:.4f}")
